    ...
    async getNextMove(ghost: Ghost, pacmanPos: Position): Promise<{ dx: number; dy: number }> {
      ...
      try {
        const response = await openai.chat.completions.create({
          ...
          messages: [{ 
            role: "system", 
            content: "You are an aggressive ghost AI focused on hunting Pacman. Prioritize direct pursuit and interception."
          },
        ...
        if (
          ...
          if (maze[newPos.y][newPos.x] === ".") {
            ...
            setMaze((prevMaze) => {
              ...
              newMaze[newPos.y] =
                newMaze[newPos.y].substring(0, newPos.x) + " " + newMaze[newPos.y].substring(newPos.x + 1)
              return newMaze
            })
          }
          return newPos
      ...
      <div className="relative" style={{ width: GRID_SIZE * CELL_SIZE, height: GRID_SIZE * CELL_SIZE }}>
        {maze.map((row, y) =>
          row.split("").map((cell, x) => (
            <div
              key={`${x}-${y}`}
              className={`absolute ${cell === "#" ? "bg-blue-800" : cell === "." ? "bg-yellow-500 rounded-full" : ""}`}
              style={{
