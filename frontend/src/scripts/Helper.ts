...
export function getFlightData(
    flightStats: flight_data_structure,
    extraStats: flight_extra_structure,
    data: flight_data_structure
) {
    ...
    extraStats.flightProgress = constrain(
        lerp(
            new Date().getTime(),
            new Date(flightStats.DEPARTURE_TIME).getTime(),
            new Date(flightStats.ARRIVAL_TIME).getTime()
        ),
        0,
        1
    );
